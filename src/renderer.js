...
function setStatus(msg, isErr = false) {
  statusText.innerHTML = isErr ? `<span style="color:#b44">${msg}</span>` : msg;
}
...
async function ensureLocalScreen() {
  ...
  await track.applyConstraints({ width: { ideal: 7680 }, height: { ideal: 4320 }, frameRate: { ideal: 60, max: 60 } }).catch(() => {});
...
async function startCall() {
  ...
  if (peerId === selfId) { setStatus('Нельзя позвонить самому себе.', true); return; }
...
function send(payload) {
  if (!socket || socket.readyState !== WebSocket.OPEN) {
    setStatus('Нет подключения к signaling server.', true); return;
  }
  socket.send(JSON.stringify({ ...payload, from: selfId }));
}

// ── Socket ────────────────────────────────────────────────────────────────────
function connectSocket(url) {
  serverTag.textContent = url;
  socket = new WebSocket(url);

  socket.addEventListener('open', () => {
    socket.send(JSON.stringify({ type: 'register', id: selfId }));
  });

  socket.addEventListener('message', async (e) => {
    const msg = JSON.parse(e.data);
    try {
      if (msg.type === 'registered') setStatus('Готово. Поделитесь ID и нажмите «Позвонить».');
      ...
      setStatus(e.message || 'Ошибка сигналинга.', true);
    }
  });

  socket.addEventListener('close', () => setStatus('Связь с сервером разорвана.', true));
  socket.addEventListener('error', () => setStatus('Ошибка подключения к серверу.', true));
}
...
(async function init() {
  ...
  selfId = profile.id;
  selfIdEl.textContent = selfId;
  connectSocket(config.signalServerUrl);
})();
