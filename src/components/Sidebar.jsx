    ...
    <aside className="flex flex-col gap-[8px] overflow-hidden min-h-0">
      ...
      <div className="bg-panel border border-border rounded-[8px] p-[12px_14px] flex flex-col gap-[8px] shrink-0">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-panel-2 border border-border rounded-[5px] p-[9px_10px] text-[13px] font-mono pointer-events-none flex items-center">
            <FormatSpan id={remoteId} />
            {!remoteId && <span className="text-[#444]">Peer ID</span>}
          </div>
          <input
            className="w-full bg-transparent border border-transparent text-transparent rounded-[5px] p-[9px_10px] outline-none text-[13px] font-mono transition-colors duration-140 focus:border-[rgba(255,255,255,0.22)]"
            value={remoteId}
            onChange={(e) =>
        ...
        <div className="flex gap-[6px]">
          {hasActiveCall ? (
            ...
            <button
              ...
              disabled={isCalling ? false : !remoteId.trim()}
            >
              <TextMorph ease={{ stiffness: 200, damping: 20 }}>
        ...
        <div className="bg-panel border border-border rounded-[8px] p-[10px_14px]">
          <div className="text-[11px] text-[#2e2e2e] font-mono overflow-hidden text-ellipsis whitespace-nowrap flex items-center gap-[6px]">
            {connectionStatus === "connected" ? (
              ...
              <AlertTriangle size={14} className="text-[#444]" />
            )}
            <TextMorph>
