...
2peer is an Electron desktop app for peer-to-peer screen sharing between exactly two participants. Video/audio flow directly between peers over WebRTC; **Supabase is used only as a signaling relay** (offer/answer/ICE exchange) — no media or persisted data passes through it.
...
- `prebuild:win` / `prebuild:mac` auto-increment the patch version in `package.json` before a build. The version is injected into `index.html` via the `{{VERSION}}` token by the `version-replace` Vite plugin (see [vite.config.js](app/vite.config.js)).
...
- [useSignaling.js](app/src/hooks/useSignaling.js) — Supabase channel send (`sendSignal`) + the `handleSignal` dispatcher that processes every inbound message type.
...
**Signaling protocol.** Each peer subscribes to its own Supabase Realtime broadcast channel `peer:<id>`. To reach a peer you open (or reuse) an outbound channel to *their* `peer:<id>` and `send` a payload. Message `type`s handled in `handleSignal`: `call`, `answer`, `candidate`, `renegotiate`, `renegotiate-answer`, `stop-broadcast`, `decline`, `cancel`, `hangup`. ICE candidates arriving before `remoteDescription` is set are buffered in `pendingIceRef` and flushed afterward. Idempotency refs (`answerProcessedRef`, `hangupProcessedRef`, `incomingProcessedRef`) guard against duplicate broadcast deliveries — reset them via `resetSignalingRefs()` when starting a new call.
...
- **Identity**: a random 12-char `[A-Z0-9]` ID (see [idUtils.cjs](app/src/utils/idUtils.cjs)) stored in `profile.json` under Electron's `userData` dir (`~/Library/Application Support/2peer/` on macOS, `%APPDATA%/2peer/` on Windows). "Update/Regenerate ID" rewrites it and re-subscribes the Supabase channel.
