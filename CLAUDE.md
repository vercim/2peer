...
2peer is an Electron desktop app for peer-to-peer screen sharing between exactly two participants. Video/audio flow directly between peers over WebRTC; **[Trystero](https://github.com/dmotz/trystero) is used only as a signaling relay** (offer/answer/ICE exchange) over two transports in parallel — BitTorrent WSS trackers **and** Nostr relays — with no media, no persisted data, and no central server involved.
...
**Signaling protocol.** Discovery runs over **two transports at once** — `@trystero-p2p/torrent` (BitTorrent WSS trackers) and `@trystero-p2p/nostr` (Nostr relays). `joinRoomMulti(roomId)` in [useSignaling.js](app/src/hooks/useSignaling.js) joins a room on every transport and returns one unified handle (`send`/`leave`/`setOn*`); a peer is reachable through whichever transport the other side can connect to, so a single blocked relay network no longer kills the call. Trystero shares one `selfId` across strategies, so the peer id is identical on both — duplicate inbound signals are absorbed by the idempotency refs below, and `action.send` to a peer absent on a transport is silently skipped. Each peer joins a room named after their own 12-char ID (`initMyRoom(selfId)`). To call someone, the caller joins the callee's room (`openCallChannel(calleeId)`) and waits for the first `onPeerJoin` from either transport (up to 30 s timeout) before sending the WebRTC offer. Signals flow through Trystero data channels; the media connection is a separate `RTCPeerConnection` managed by `usePeerConnection`. Message `type`s handled in `handleSignal`: `call`, `answer`, `candidate`, `renegotiate`, `renegotiate-answer`, `stop-broadcast`, `decline`, `cancel`, `hangup`. ICE candidates arriving before `remoteDescription` is set are buffered in `pendingIceRef` and flushed afterward. Idempotency refs (`answerProcessedRef`, `hangupProcessedRef`, `incomingProcessedRef`) guard against duplicate deliveries — reset via `resetSignalingRefs()` when starting a new call.
...
**Design tokens & accent.** All colors live in [src/colormap.css](app/src/colormap.css) (Tailwind v4 `@theme`, OKLCH), imported by [index.css](app/src/index.css) which holds the type/radius scales and fonts. Dark is the default; light overrides under `[data-theme="light"]`. Components reference colors only through token utilities (`bg-panel`, `text-text`, `bg-accent`, …) — don't hardcode hex/rgba in JSX (exception: elements layered over the always-dark video area). The brand is teal. App.jsx stores the raw accent **seed** (a hex) and sets `--color-accent-base`; `colormap.css` derives the live `--color-accent` per theme via `color-mix` (brighten on dark, darken on light), so one picked color reads on both. `normalizeAccent` migrates the legacy default (`#B9D9CC`) to the new teal; deliberately-chosen custom colors are preserved.

**Quality / bitrate.** Resolution+fps presets and default bitrates live in [src/utils/rtcConfig.js](app/src/utils/rtcConfig.js). Quality is enforced two ways that work together: SDP munging (`setMaxBandwidthInSDP` in [sdpUtils.js](app/src/utils/sdpUtils.js)) sets `b=AS` bandwidth lines on every offer/answer, and sender `encodings` are set via `applyMaxQualityEncoding` in [bitrateManager.js](app/src/utils/bitrateManager.js). Changing `streamQuality` re-applies constraints, encodings, and SDP. `rtcConfig.js` exposes one vetted ICE config (`ICE_SERVERS`, hostname-based STUN only — dead hosts and stale hardcoded IPs were removed) that is shared by **both** the media `RTCPeerConnection` and the Trystero signaling channels. There are still **no TURN servers**, so peers behind symmetric NATs may fail to connect.
